...
{																																								//	begin
	...
	{																																							//	begin
		...
		int		c1[6] = {0x00, 0x33, 0x66, 0x99, 0xcc, 0xff};
		for (int i = 0; i < height; i++)																						//	loop over all of the rows
		{																																						//	begin
			for (int j = 0; j < width; j++)																						//	loop over all of the pixels in the row
			{																																					//	begin
				ulong		red = c1[array[i][j].Red ()],																		//	get the red channel
								green = c1[array[i][j].Green ()] << 8,													//	get the green channel
								blue = c1[array[i][j].Blue ()] << 16;														//	get the blue channel
