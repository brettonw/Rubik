...
//------------------------------------------------------------------------------
EksLaserGun::EksLaserGun (Real rat, Real ran, const PiecePtr &own, CPoint muz,	//	constructor
...
//------------------------------------------------------------------------------
EksLaserGun::~EksLaserGun (void)																								//	destructor
{																																								//	begin
}																																								//	end

//------------------------------------------------------------------------------
void	EksLaserGun::Shoot (void)																									//	shoot the projectile
{																																								//	begin
/*
...
*/
	...
	{
		Bool	damageit = TRUE;
		Player *victOwn = victim->GetOwner ();
		if (victOwn && (victOwn->Team () == TEAM_EKSEKAREN))
			damageit = FALSE;
		if (damageit == TRUE)
		{
			Vector_2d	victVel;
	...
	gView->Install (new BeamSeq (time, color, rate * R(0.5), muzzle, owner), UNDER_LAYER);
}																																								//	end
