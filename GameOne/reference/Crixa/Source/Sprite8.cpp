...
#define OLD_BLIT 0																															//	whether to use C++ copy or new asm
...
{																																								//	begin
	...
	{																																							//	begin
		...
		{																																						//	begin
			...
			for (uint j = 0; j < rows[i].numRuns; j++)																//	loop over all of the runs
			{																																					//	begin
...
/******************************************************************************\
	Draw -- Draws Sprite with Optional Opaqueness Mask
\******************************************************************************/
...
{																																							//	begin
	...
	{																																							//	begin
		...
		{																																						//	begin
			...
			#if OLD_BLIT
				for (int k = 0; k < run->length; k++)																		//	loop over all of the pixels in the run
					pixels[k + run->x] = run->pixels[k];																	//	copying the new pixel value
			#else
				{
					len = run->length;																										//	get the run length
					isrc = (long*) run->pixels,																						//	get a pointer to the source pixels
...
{																																								//	begin
	...
	int		len;																																		//	used for length computation
	long	*isrc, *idst;
	...
	{																																							//	begin
		...
		{																																						//	begin
			...
			#if OLD_BLIT
				for (int k = runStart; k < runStop; k++)																//	loop over all of the visible pixels
					pixels[k + run->x] = run->pixels[k];																	//	copying the new pixel
			#else
				{																																				//	begin
					len = runStop - runStart;																							//	compute the clipped length
					if (len > 0)																													//	if the clipped length is greater than zero
					{																																			//	begin
						isrc = (long*) &(run->pixels[runStart]),														//	get a pointer to the source pixels
...
/******************************************************************************\
	DrawAdd -- Draws Additive Only version of Sprite
\******************************************************************************/
...
{																																								//	begin
	pPtr			pixels = gDisplay->PPtr (x, y);																			//	get a pointer to the screen space
//	int				len;																															// used for assembly blit length
//	long			*isrc, *idst;																											// used for assembly blit src and dest pointers
	...
	{																																							//	begin
		...
		{																																						//	begin
			Run		*run = row->runs[j];																								//	get a pointer to the current run
			for (int k = 0; k < run->length; k++)																			//	loop over all of the pixels in the run
				pixels[k + run->x] = pixels[k + run->x] + run->pixels[k];								//	scaling the existing screen pixel and adding the new one
		}																																						//	end of runcounter
		pixels += gDisplay->Stride ();																							//	increment the pixel pointer to the next scanline
	}																																							//	end of rowcounter
}																																								//	end
...
//------------------------------------------------------------------------------
void	Sprite::DrawAddClipped (int x, int y)																			//	draw the sprite clipped to the screen bound
{																																								//	begin
	...
	int		yStart = 0,																															//	which row to start with
				...
				xStart = 0,																															//	which column to start with
				xStop = width;																													//	which column to end with
//	int		len;																																	//	used for assembly length computation
//	long	*isrc, *idst;																													//	used for assembly src and dest pointers
	...
	{																																							//	begin
		...
		{																																						//	begin
			...
			if (run->x + runStop > xStop)																							//	if the run continues off the right edge
				runStop = xStop - run->x;																								//	clip the end off to the right edge
			for (int k = runStart; k < runStop; k++)																	//	loop over all of the visible pixels
				pixels[k + run->x] = pixels[k + run->x] + run->pixels[k];								//	scaling down the existing pixel and blending in the new pixel
		}																																						//	end
		pixels += gDisplay->Stride ();																							//	skip the pixel pointer to the next scan line
	}																																							//	end
}																																								//	end
...
/******************************************************************************\
	DrawFlash -- Draws White version of the sprite
\******************************************************************************/
