...
TChain<Real>::ContainsPoint (const TPoint2<Real>& point) const
{
    ...
    for (uInt i = 0; i < iSize; i++)
    {
        ...
        uInt                    j = (i + 1) % iSize;
        const TPoint2<Real>&    ptA = at (i);
        const TPoint2<Real>&    ptB = at (j);
...
TChain<Real>::TestSimple (void) const
{
    ...
    // vertex count
    uInt2           iEdgeCount = static_cast<uInt2> (size ());
    ...
    for (uInt2 i = 0; i < iEdgeCount; i++)
    {
        ...
        TEdge2<Real>    edgeA (at (i), at (iNext));
...
{
    // fetch the size of the input chain
    uInt2           iVertexCount = static_cast<uInt2> (size ());
    ...
    if (m_bClosed)
    {
        ...
        do
            b = at (--i);
        while ((TPoint2<Real>::FuzzyEquals (a, b, rEpsilon)) and (i > 1));
        a = at (--i);
        // Assert (i != 0);
        iStart = 0;
    }
    ...
    for (uInt2 i = iStart; i < iVertexCount; i++)
    {
        ...
        if ((NormalizedLineTest (a, b, c, rEpsilon) != COLINEAR) and
            (TPoint2<Real>::FuzzyNotEquals (a, b, rEpsilon)) and
            (TPoint2<Real>::FuzzyNotEquals (a, c, rEpsilon)) and
            (TPoint2<Real>::FuzzyNotEquals (b, c, rEpsilon)))
        {
    ...
    else
    {
        ...
        if (TPoint2<Real>::FuzzyEquals (front (), back (), rEpsilon))
...
TChain<Real>::PunchHole (const TChain<Real>& holeChain)
{
    ...
    const TPoint2<Real>&   ptRayOrigin = holeChain[0];
    ...
    for (uInt i = 0; i < iSize; i++)
    {
        // fetch the current and next points in the chain
        uInt    j = (i + 1) % iSize;
        TPoint2<Real>& ptA = at (i);
        TPoint2<Real>& ptB = at (j);
        
        // compute the intersection "time" for a right ray with the edge AB
        Real            rT = IntersectRightRaySegment (ptRayOrigin, ptA, ptB);
        ...
        if ((rT > 0) and (rT < rBestT))
        {
            rBestT = rT;
            iBestIndex = j;
            // Assert (LineTest (ptA, ptRayOrigin + (vRayDirection * rBestT), ptB) == COLINEAR);
        }
    }
    
    // compute the intersection point from the best found intersection
    // Assert (iBestIndex != 0xffffffff);
    TPoint2<Real>           ptIntersection = ptRayOrigin + (vRayDirection * rBestT);
    
    // use the intersection point to compute the new UV coordinates
    TPoint2<Real>&         ptA = at (((iBestIndex - 1) + iSize) % iSize);
    TPoint2<Real>&         ptB = at (iBestIndex);
    Real                    rT;
    Real                    rDenominator = ptB.y - ptA.y;
    if (FuzzyNotEquals<Real> (rDenominator, 0))
        rT = (ptIntersection.y - ptA.y) / rDenominator;
    else
    {
        rDenominator = ptB.x - ptA.x;
        // Assert (FuzzyNotEquals<Real> (rDenominator, 0));
        rT = (ptIntersection.x - ptA.x) / rDenominator;
    }
    // Assert ((rT >= 0) and (rT <= 1));
    // Assert (ptA + ((ptB - ptA) * rT) == ptIntersection);
    ...
    insertionIter = insert (insertionIter, TPoint2<Real> (ptIntersection, uv));
    insertionIter = insert (insertionIter, TPoint2<Real> (ptIntersection, uv));
    insertionIter++;
...
TChain<Real>::SetUVCoordinates (const TBound2<Real>& uvBound)
{
    ...
    while (iter != iterEnd)
    {
        ...
        TPoint2<Real>&     vertex = *iter++;
        
        // compute the uv coordinate relative to the bound and set it
        TVector2<Real>      vDelta = vertex - ptLowerLeft;
...
TChain<Real>::Expanded (Real rAdd, Real rTesselationLimit) const
{
    ...
    uInt            iSize = size ();
    TPoint2<Real>  ptA = at (0);
    TVector2<Real>  vXA = ptA - at (iSize - 1);
    ...
    for (uInt i = 0; i < iSize; i++)
    {
        ...
        const TPoint2<Real>&   ptB = at ((i + 1) % iSize);
    
        // compute the vector that will be used for expansion
        TVector2<Real>          vAB = ptB - ptA;
        ...
        if (FuzzyNotEquals<Real> (rAngleTest, 0))
        {
            ...
            expandedChain.push_back (ptA + (vPrev * rAdd));
            ...
            if (rAngleTest > REAL_EPSILON)
            {
                ...
                TesselateAngle (expandedChain, ptA, rAdd, vPrev, vNext, rTesselationLimit);
            }
            
            // store the second edge point
            expandedChain.push_back (ptA + (vNext * rAdd));
        }
