...
struct EndPoint
{
    typedef typename TChainList<Real>::iterator TChainListIterator;
    TChainListIterator          chainIter;
    Real                        rOffsetA;
    Real                        rOffsetB;
    bool                        bFront;
    /* void */                  EndPoint (TChainListIterator _chainIter, Real _rOffsetA, Real _rOffsetB, bool _bFront) :
...
{
    typedef typename TChainList<Real>::iterator TChainListIterator;
    bool    operator () (const TChainListIterator& lhs, const TChainListIterator& rhs) const
    {
    ...
    if (chainsOut.size () > 1)
    {
        ...
        if (chainsOut.size () > 1)
        {
            ...
            while (chainIter != chainEnd)
            {
                ...
                Assert (LineTest (edge.a, edge.b, chainIter->front ()) == COLINEAR);
                TVector2<Real>  compareVec = chainIter->front () - edge.a;
                Real            rOffsetFront = compareVec DOT ab;
                
                // get the last point of the chain, and qualify it with 
                // respect to the clip line
                Assert (LineTest (edge.a, edge.b, chainIter->back ()) == COLINEAR);
                compareVec = chainIter->back () - edge.a;
...
ClipToLine (TChainList<Real>& chainsOut, const TChainList<Real>& chainsIn, const TEdge2<Real>& edge)
{
    ...
    while (inputChainIter != inputChainEnd)
    {
        ...
        uInt2               iVertexCount = static_cast<uInt2> (chain.size ());
        ...
        TPoint2<Real>       c = chain[iVertexCount - 1];
        TPoint2<Real>       d = chain[0];
        sInt                fTestC = LineTest (edge, c);
        sInt                fTestD = LineTest (edge, d);
        ...
        for (uInt2 i = 1; i <= iVertexCount; i++)
        {
            ...
            // right or left of c
            if ((iAction == SPECIAL_CASE) and (LineTest (lastC, c, d) == TURNS_LEFT))
                iAction = PRE_EMIT;
